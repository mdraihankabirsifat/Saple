...
test('authentication middleware verifies JWT and attaches current database identity', async () => {
  ...
  const token = jwt.sign(
    { userId: 8, role: 'USER', tokenVersion: 3 },
  ...
  userRepository.findAuthorizationById = async () => ({
    accountRole: 'ADMIN', accountStatus: 'ACTIVE', tokenVersion: 3
  });
...
test('authentication middleware rejects a suspended account even with a valid JWT', async () => {
  ...
  userRepository.findAuthorizationById = async () => ({
    accountRole: 'USER', accountStatus: 'SUSPENDED', tokenVersion: 3
  });
  const response = createResponse();
  let nextCalled = false;
...
test('authentication middleware rejects a token-version mismatch with 401', async () => {
  process.env.JWT_SECRET = 'test-only-secret-that-is-long-enough-for-tests';
  const token = jwt.sign(
    { userId: 8, role: 'USER', tokenVersion: 2 },
    process.env.JWT_SECRET,
    {
      algorithm: authConfig.JWT_ALGORITHM,
      issuer: authConfig.JWT_ISSUER,
  ...
  userRepository.findAuthorizationById = async () => ({
    accountRole: 'USER', accountStatus: 'ACTIVE', tokenVersion: 3
  });
  const response = createResponse();
  let nextCalled = false;
  ...
  assert.equal(response.statusCode, 401);
  assert.equal(response.body.message, 'Authentication token has been revoked');
  assert.equal(nextCalled, false);
});
