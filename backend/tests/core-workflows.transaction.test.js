...
test('review child-insert failure rolls back the parent submission', async () => {
  ...
  const client = mockClient(async () => {
    ...
    if (execution === 5) return { rows: [{ submissionId: 12 }] };
...
test('interview child-insert failure rolls back the parent submission', async () => {
  ...
  const client = mockClient(async () => {
    ...
    if (execution === 3) return { rows: [{ roleId: 1 }] };
    if (execution === 4) return { rows: [{ verificationId: 9 }] };
    if (execution === 5) return { rows: [{ submissionId: 13 }] };
    throw new Error('child insert failed');
  });
  database.getClient = async () => client;
...
test('verification and report decisions roll back when their update fails', async () => {
  for (const operation of [
    () => verificationRepository.decideVerification({
      verificationId: 5, reviewerUserId: 6, status: 'VERIFIED', rejectionReason: null
    }),
    () => reportRepository.updateReportStatus({
      reportId: 3, resolverUserId: 6, status: 'RESOLVED',
      resolutionNote: 'Done', allowedPreviousStatuses: ['OPEN']
    })
  ]) {
    let execution = 0;
    const client = mockClient(async () => {
      execution += 1;
      if (execution === 1) {
        return { rows: [{ verificationStatus: 'PENDING', roleId: 1, reportStatus: 'OPEN' }] };
      }
